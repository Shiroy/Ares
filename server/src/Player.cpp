...
Player::Player(unsigned int m_id,
...
AresProtocol::AresMessage Player::getCreationMessage(const bool setMyself) {
  ...
  for (auto reflectorValue : reflectors) {
    ...
    switch (reflectorValue.second.get_value_type()) {
      case BOOLEAN :pktReflector->set_boolean(reflectorValue.second.get_value_boolean());
        break;
      case NUMBER :pktReflector->set_number(reflectorValue.second.get_value_number());
        break;
      case STRING :pktReflector->set_string(reflectorValue.second.get_value_str());
        break;
      default:continue;
    }
  }
