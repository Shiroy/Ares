...
void Tilemap::draw(sf::RenderTarget &target, sf::RenderStates states) const {
  ...
  vertexArray.reserve(
      (static_cast<int>(viewport.width) / tile_width) * (static_cast<int>(viewport.height) / tile_height));
  ...
  for (auto layer : layers) {
    ...
    for (int row = start_row; row <= end_row; row++) {
      for (int column = start_column; column <= end_column; column++) {
        ...
        sf::Vector2f textureCoord = tile_value_to_tex_coord(tile_value, tileset_row_length, tile_width,
                                                            tile_height);
...
sf::Vector2f
Tilemap::tile_value_to_tex_coord(int tile_value, int tileset_row_length, int tile_width, int tile_height) const {
  return sf::Vector2f((tile_value % tileset_row_length) * tile_width,
                      (tile_value / tileset_row_length) * tile_height);
}
